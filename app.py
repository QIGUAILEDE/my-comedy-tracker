...
# 🚀 核心升级：通过 CSS Media Queries 实现双端无缝响应
st.markdown(f"""
<style>
    ...
    /* 魔法逻辑：
       我们在 Python 层同时生成了「桌面网格」和「手机时间轴」两套 UI。
       通过这层 CSS，当屏幕 > 800px 时，隐藏时间轴；当屏幕 < 800px 时，隐藏网格。
       完全由浏览器原生接管，极致丝滑。
    */
...
if menu == "📅 日程排期":
    if not total_df.empty:
        ...
        # 【双生 UI 架构】：我们将同时渲染电脑版和手机版，由 CSS 媒体查询在浏览器端决定显示哪个
        
        # 1. 桌面专属 UI：网格日历 (包裹在 css class 容器中)
        ...
        # 2. 手机专属 UI：垂直时间轴 (包裹在 css class 容器中)
            ...
            if raw_text.strip():
                with st.spinner("解析中..."):
                    try:
                        ...
                        backticks = chr(96) * 3 
                        if ai_content.startswith(backticks): ai_content = ai_content.split('\n', 1)[1].rsplit('\n', 1)[0].strip()
                        if ai_content.startswith("json"): ai_content = ai_content[4:].strip()
                        st.session_state['parsed_data'] = json.loads(ai_content)
                    except: st.error("解析失败，请检查输入或 API 状态。")
